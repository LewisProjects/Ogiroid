...
class Welcome(Cog):
    ...
    async def on_member_join(self, member):
        ...
        embed = disnake.Embed(title="Welcome!",
        ...
        embed.set_author(name=f"Welcome, {member.name}", url=f"{member.avatar.url}", icon_url=f"{member.avatar.url}", )
    ...
    async def on_member_remove(self, member):
        chan = self.get_channel(905183354930995320)
        embed = disnake.Embed(title="Goodbye :(",
        ...
        embed.set_author(name="Member Left!", url=f"{member.avatar.url}", icon_url=f"{member.avatar.url}", )
