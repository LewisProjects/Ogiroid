...
class Math(commands.Cog):
    ...
    async def math(self, inter, equation):
        ...
        try:
            ...
            await errorEmb(inter, "You used invalid syntax in your equation.\n"
            ...
            await errorEmb(inter, "Are you sure that is a valid equation?\n"
                                  "If you need help use the ``/mathhelp`` command.\n"
                                  "If this is a bug report it with the ``/reportbug`` command."
                           )
        except Exception as e:
            await errorEmb(inter, f"{e}\n"
    ...
    async def math_help(self, inter):
        await QuickEmb(inter, "The following operations are supported:\n"
                              " `+` (addition)\n `-` (subtraction)\n `*` (multiplication)\n `/` (division) \n"
                              "`//` (floor division)\n `%` (modulo)\n `^` (exponentation)\n `!` (factorial)\n"
                              "Aswell as these:\n"
                              " `sqrt` | `cbrt` | `log` | `log10` | `ln` | `rad` | `sin` | `cos` | `tan` | `asin` | `acos` | `atan`").send()
