    ...
    async def on_slash_command_error(self, inter: ApplicationCommandInteraction, error):
        try:
            ...
            if isinstance(error, CommandNotFound):
                ...
                return await errorEmb(inter,
                                      f'You called the {inter.application_command.name} command with too many arguments.')
                ...
                traceback_nice = "".join(
                    traceback.format_exception(type(error), error, error.__traceback__, 4)).replace(
                    "```", "```"
                )
