...
class Fun(commands.Cog):
    ...
    async def youtube(self, ctx):
        ...
        if ctx.author.voice:
            ...
            embed = disnake.Embed(title="YouTube",
                                  description=f"Click __[here]({link})__ to start the YouTube together session.",
                                  color=0xFF0000, timestamp=datetime.utcnow(), )
            embed.set_footer(text=f"Command issued by: {ctx.author.name}", icon_url=ctx.author.avatar, )
    ...
    async def joke(self, inter):
        ...
        embed.set_footer(text=f"Command issued by: {inter.author.name}", icon_url=inter.message.author.avatar, )
        await inter.send(embed=embed)

    @commands.slash_command(name="trigger", brief="Trigger", description="For when you're feeling triggered.", )
    ...
    @commands.slash_command(name="sus", brief="Sus-Inator4200", description="Check if your friend is kinda ***SUS***", )
    @commands.cooldown(1, 10, commands.BucketType.user)
    ...
    @commands.slash_command(name="beer",
                            description="Give someone a beer! 🍻")  # Credit: AlexFlipNote - https://github.com/AlexFlipnote
    ...
    async def askogiroid(self, ctx):
        async with ctx.author.typing():
            intro = disnake.Embed(title="Ogiroid", description=f"Hello {ctx.author.mention}!", color=0xFFFFFF, )
            intro.set_thumbnail(
                ...
                while aki.progression <= 80:
                    ...
                    if msg.content.lower() in ["b", "back"]:
                        ...
                        try:
                            ...
                            continue
                aki.win()
                answer = disnake.Embed(title=f"Your character: {aki.first_guess['name']}",
