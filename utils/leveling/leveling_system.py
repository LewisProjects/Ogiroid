...
import aiosqlite
from disnake import Guild, Member, Message, MessageType, Role
from disnake.ext.commands import AutoShardedBot, Bot, BucketType, CooldownMapping

from .announcement import LevelUpAnnouncement
from .decorators import db_file_exists, levels_exists, verify_levels_integrity
from .errors import *
...
class LevelingSystem:
    """A local disnake.py leveling system powered by SQLite
    ...
    bot: Union[:class:`disnake.ext.commands.AutoShardedBot`, :class:`disnake.ext.commands.Bot`]
    ...
    def __init__(self, rate: int = 1, per: float = 60.0, awards: Optional[Dict[int, List[RoleAward]]] = None, **kwargs):
        ...
        self.level_up_announcement: Union[LevelUpAnnouncement, Sequence[LevelUpAnnouncement]] = kwargs.get(
    ...
    class Bonus:
        ...
        async def on_message(message):
            await lvl.award_xp(amount=[15, 25], message=message, bonus=LevelingSystem.Bonus([nitro_booster, associate_role], 20, multiply=False))
        ```
            ...
            if len(role_ids) >= 1:
                ...
                if multiply and bonus_amount > 3:
                    raise LevelingSystemError(
                        'Parameter "bonus_amount" cannot be greater than 3 when parameter "multiply" is True'
                    )

            else:
                raise LevelingSystemError("When setting the role_ids for bonus XP, the role ID sequence cannot be empty")
    ...
    def get_xp_for_level(level: int) -> int:
        ...
        Raises
        ------
        - `LevelingSystemError`: The level specified does not exist
        ...
        try:
            ...
            raise LevelingSystemError(f"Levels only go from 0-100, {level} is not a valid level")
    ...
    def create_database_file(path: Optional[str] = None) -> None:
        ...
        - `LevelingSystemError`: The path does not exist or the path points to a file instead of a directory
        ...
        if os.path.exists(path) and os.path.isdir(path):
            database_file = os.path.join(path, "LevelingSystem.db")
            ...
            raise LevelingSystemError(
                f"The path {path!r} does not exist or that path directs to a file when it is suppose to path to a directory"
            )
    ...
    def backup_database_file(self, path: str, with_timestamp: bool = False) -> None:
        ...
        Raises
        ------
        - `LevelingSystemError`: Path doesn't exist or points to another file
        ...
        if os.path.exists(path) and os.path.isdir(path):
            if not with_timestamp:
                database_file = os.path.join(path, "DisnakeLevelingSystem__backup.db")
                shutil.copyfile(src=self._database_file_path, dst=database_file)
            else:
                dt = datetime.now()
                dt_str = dt.strftime("%Y_%b_%d__%I_%M_%S_%p__%f")
                database_file = os.path.join(path, "DisnakeLevelingSystem__backup(%s).db" % dt_str)
                shutil.copyfile(src=self._database_file_path, dst=database_file)
        else:
            raise LevelingSystemError(
                f'When attempting to backup the database file, the path "{path}" does not exist or points to another file'
            )
    ...
    def _determine_no_xp(self, message: Message) -> bool:
        ...
        if self.no_xp_roles:
            all_member_role_ids = [role.id for role in message.author.roles]  # type: ignore / will always be `disnake.Member` because all DM messages are ignored by the lib
    ...
    async def _execute_transfer(db_from: "Transfer", db_to: "Transfer", guild_id: int) -> None:  # type: ignore
        ...
        try:
            ...
            raise LevelingSystemError('One of the databases is missing the "leaderboard" table when attempting to transfer')
        else:
            OLD_PRAGMA_LAYOUT = [
            ...
            if all([old_pragma_check == OLD_PRAGMA_LAYOUT, new_pragma_check == NEW_PRAGMA_LAYOUT]):
                ...
                if count_result[0] == 0:
                    ...
                    for data in from_result:
                        ...
                        await db_to.cursor.executemany(LevelingSystem._QUERY_NEW_MEMBER, to_execute)
                        await db_to.connection.commit()
                        print("Transfer complete")
                else:
                    raise LevelingSystemError(
                        "When transferring the data to the new database file (created file using v0.0.2+), that database file must contain no records"
                    )
            else:
                raise LevelingSystemError(
                    'The "transfer" method is only to be used with transferring the data from the database file from version 0.0.1. If you were already using a database file from version 0.0.2+, there is no need to use this method'
                )
    ...
    def transfer(old: str, new: str, guild_id: int) -> None:
        ...
        See the following link about transfers: https://github.com/Defxult/disnakeLevelingSystem#migrating-from-v001-to-v002
        ...
        - `LevelingSystemError`: One of the databases is missing the "leaderboard" table. A v0.0.2+ database file contains records, or there was an attempt to transfer records from a v0.0.2+ file to another v0.0.2+ file

            .. added:: v0.0.2
        """
        loop = asyncio.get_event_loop()
        transfer_from = LevelingSystem._get_transfer(old, loop)
        transfer_to = LevelingSystem._get_transfer(new, loop)
        loop.run_until_complete(LevelingSystem._execute_transfer(transfer_from, transfer_to, guild_id))
    ...
    async def add_record(self, guild_id: int, member_id: int, member_name: str, level: int) -> None:
        ...
        Raises
        ------
        - `LevelingSystemError`: The value given from a parameter was not of the correct type or "level" was not 0-100

            .. added:: v1.0.1
        """
        if all([isinstance(guild_id, int), isinstance(member_id, int), isinstance(level, int)]):
            if not (0 <= level <= 100):
                raise LevelingSystemError('Parameter "level" must be from 0-100')
            ...
            raise LevelingSystemError("All parameters that expect an int were not of type int")
    ...
    async def insert(
        ...
        file using :meth:`LevelingSystem.backup_database_file()` first. If you don't have any records in your LevelingSystem database file, then there's no need to create a backup

        Parameters
        ----------
        bot: Union[:class:`disnake.ext.commands.Bot`, :class:`disnake.ext.commands.AutoShardedBot`]
        ...
        Raises
        ------
        - `LevelingSystemError`: The value given from a parameter was not of the correct type. The :param:`users` dict was empty. Or your bot is not in the guild associated with :param:`guild_id`

            .. added:: v1.0.1
        """
        # Perform the necessary checks to ensure the proper values will be added to the database
        if not isinstance(guild_id, int):
            raise LevelingSystemError(f'Parameter "guild_id" expected int, got {guild_id.__class__.__name__}')
        if not users:
            raise LevelingSystemError('The "users" dict cannot be empty')
        for k, v in users.items():
            if not isinstance(k, int) or not isinstance(v, int):
                raise LevelingSystemError('All keys and values in the "users" dict must be of type int')
        ...
        if guild:
            ...
            for user_id, user_level_or_xp in users.items():
                ...
                if member:
                    if not overwrite and await self.is_in_database(user_id, guild):
                        ...
                        if using == "levels":
                            ...
                            if xp < 0:
                                xp = 0
                            elif xp > LEVELS_AND_XP[str(MAX_LEVEL)]:
                                xp = LEVELS_AND_XP[str(MAX_LEVEL)]
                            await self.set_level(member, _find_level(xp))
                            successfully_added.append(member)

                        else:
                            raise LevelingSystemError(f'Parameter "using" expected "levels" or "xp", got {using!r}')
                else:
                ...
                if show_results:
                    stats = cleandoc(
                        f"""
                        ----------------------------------------
                        Disnake Leveling System - Insert Results
                        ----------------------------------------
                    ...
                    if successfully_added:
                        ...
                        data: List[MemberDataStringRepr] = [
                            str(await self.get_data_for(stored_member)) for stored_member in successfully_added
                        ]
                        joined_data = "\n".join(data)
                        stats += f"\n\nThe below {len(successfully_added)} user(s) are now apart of the Disnake Leveling System and are represented as a MemberData object\n{joined_data}"
            ...
            raise LevelingSystemError(f"Your bot is not in guild {guild_id}")

    def get_awards(
        self, guild: Optional[Union[Guild, int]] = None
    ) -> Optional[Union[Dict[int, List[RoleAward]], List[RoleAward]]]:
        """Get all :class:`RoleAward`'s or only the :class:`RoleAward`'s assigned to the specified guild

        Parameters
        ----------
        guild: Optional[Union[:class:`disnake.Guild`, :class:`int`]]
    ...
    async def add_xp(self, member: Member, amount: int) -> None:
        ...
        - `NotConnected`: Attempted to use a method that requires a connection to a database file
        - `LevelingSystemError`: Parameter "amount" was less than or equal to zero. The minimum value is 1

            .. added:: v0.0.2
        """
        if amount <= 0:
            raise LevelingSystemError('Parameter "amount" was less than or equal to zero. The minimum value is 1')
    ...
    async def set_level(self, member: Member, level: int) -> None:
        ...
        - `ImproperLeaderboard`: Leaderboard table was altered. Components changed or deleted
        - `NotConnected`: Attempted to use a method that requires a connection to a database file
        - `LevelingSystemError`: Parameter "level" was not from 0-100
    ...
    async def change_cooldown(self, rate: int, per: float) -> None:
        ...
        - `ImproperLeaderboard`: Leaderboard table was altered. Components changed or deleted
        - `NotConnected`: Attempted to use a method that requires a connection to a database file
        - `LevelingSystemError`: The rate or per value was not greater than zero
        """
        if rate <= 0 or per <= 0:
            raise LevelingSystemError("Invalid rate or per. Values must be greater than zero")
        self._cooldown = CooldownMapping.from_cooldown(rate, per, BucketType.member)
            ...
            if result:
                ...
                for db_id, db_name in zip(database_ids, database_names):
                    ...
                    await cursor.executemany(
                        "UPDATE leaderboard SET member_name = ? WHERE member_id = ? AND guild_id = ?", to_execute
                    )
                    await self._connection.commit()  # type: ignore
    ...
    async def wipe_database(self, guild: Optional[Guild] = None, *, intentional: bool = False) -> None:
        ...
        if intentional:
            if guild:
                await self._cursor.execute("DELETE FROM leaderboard WHERE guild_id = ?", (guild.id,))  # type: ignore
            else:
                await self._cursor.execute("DELETE FROM leaderboard")  # type: ignore
            await self._connection.commit()  # type: ignore
        else:
            raise FailSafe
    ...
    async def reset_everyone(self, guild: Union[Guild, None], *, intentional: bool = False) -> None:
        ...
        Sets EVERYONE'S XP, total XP, and level to zero in the database. Can specify which guild to reset

        Parameters
        ----------
        guild: Union[:class:`disnake.Guild`, :class:`None`]
    ...
    async def export_as_json(self, path: str, guild: Union[Guild, None]) -> None:
        ...
        - `ImproperLeaderboard`: Leaderboard table was altered. Components changed or deleted
        - `NotConnected`: Attempted to use a method that requires a connection to a database file
        - `LevelingSystemError`: The path does not exist or does not point to a directory
        ...
        if os.path.exists(path) and os.path.isdir(path):
            ...
            raise LevelingSystemError(f"The path {path!r} does not exist or does not point to a directory")
    ...
    async def raw_database_contents(self, guild: Optional[Guild] = None) -> List[Tuple[int, int, str, int, int, int]]:
        ...
        """
        if guild:
            return await self._connection.execute_fetchall("SELECT * FROM leaderboard WHERE guild_id = ?", (guild.id,))  # type: ignore
        else:
            return await self._connection.execute_fetchall("SELECT * FROM leaderboard")  # type: ignore
    ...
    async def remove_from_database(self, member: Union[Member, int], guild: Optional[Guild] = None) -> bool:
        ...
        Parameters
        ----------
        member: Union[:class:`disnake.Member`, :class:`int`]
        ...
        - `LeaderboardNotFound`: Table "leaderboard" in the database file is missing
        - `ImproperLeaderboard`: Leaderboard table was altered. Components changed or deleted
        - `LevelingSystemError`: Parameter :param:`member` was not of type :class:`disnake.Member` or :class:`int`
        - `NotConnected`: Attempted to use a method that requires a connection to a database file
        ...
        if isinstance(member, (Member, int)):
            ...
            if removable:
                query = (
                    "DELETE FROM leaderboard WHERE member_id = ? AND guild_id = ?"
                    if guild
                    else "DELETE FROM leaderboard WHERE member_id = ?"
                )
            ...
            raise LevelingSystemError(f'Parameter "member" expected disnake.Member or int, got {member.__class__.__name__}')
        ...
        - `LevelingSystemError`: Parameter :param:`member` was not of type :class:`disnake.Member` or :class:`int`
            ...
            .. changes::
                v1.0.0
                    Added :param:`guild`
        """
        if not isinstance(member, (Member, int)):
            raise LevelingSystemError(f'Parameter "member" expected disnake.Member or int, got {member.__class__.__name__}')
        arg = member.id if isinstance(member, Member) else member
        query = (
            "SELECT * FROM leaderboard WHERE member_id = ? AND guild_id = ?"
            if guild
            else "SELECT * FROM leaderboard WHERE member_id = ?"
        )
    ...
    async def get_record_count(self, guild: Optional[Guild] = None) -> int:
        ...
        """
        if guild:
            await self._cursor.execute("SELECT COUNT(*) from leaderboard WHERE guild_id = ?", (guild.id,))  # type: ignore
        else:
            await self._cursor.execute("SELECT COUNT(*) from leaderboard")  # type: ignore
    ...
    @levels_exists
    @verify_levels_integrity
    async def each_member_data(
        self, guild: Guild, sort_by: Optional[str] = None, limit: Optional[int] = None
    ) -> List[MemberData]:
        """|coro|
        ...
        - `ImproperLeaderboard`: Leaderboard table was altered. Components changed or deleted
        - `NotConnected`: Attempted to use a method that requires a connection to a database file
        - `LevelingSystemError`: The value of :param:`sort_by` was not recognized or :param:`guild` was not of type :class:`disnake.Guild`
        ...
        if not isinstance(guild, Guild):
            raise LevelingSystemError(f'Parameter "guild" expected disnake.Guild got {guild.__class__.__name__}')
        else:
            ...
            if not sort_by:
                ...
                if sort_by in ("name", "level", "xp", "rank"):
                    ...
                    raise LevelingSystemError(
                        f'Parameter "sort_by" expected "name", "level", "xp", or "rank", {sort_by!r} was not recognized'
                    )
    ...
    async def sql_query_get(
        self, sql: str, parameters: Optional[Tuple[Union[str, int]]] = None, fetch: Union[str, int] = "ALL"
        ...
        - `ImproperLeaderboard`: Leaderboard table was altered. Components changed or deleted
        - `NotConnected`: Attempted to use a method that requires a connection to a database file
        - `LevelingSystemError`: Argument "fetch" was the wrong type or used an invalid value
        ...
        if isinstance(fetch, str):
            ...
            if fetch in ("ALL", "ONE"):
                ...
                raise LevelingSystemError(f"Fetch {fetch!r} not recognized")
        elif isinstance(fetch, int):
            if fetch > 0:
                async with self._connection.execute(sql, parameters) as cursor:  # type: ignore
                    return await cursor.fetchmany(fetch)  # type: ignore
            else:
                raise LevelingSystemError('Argument "fetch" must be greater than zero')
        else:
            raise LevelingSystemError(f'Argument "fetch" needs to be str or int, got {fetch.__class__.__name__}')
    ...
    async def _refresh_name(self, message: Message) -> None:
        """|coro| If the members current database name doesn't match the name that's on disnake, update the name in the database
    ...
    async def _handle_level_up(self, message: Message, md: MemberData, leveled_up: bool) -> None:
        ...
        """
        if leveled_up:
            member: Member = message.author  # type: ignore / `.author` will be :class:`disnake.Member` (lib doesn't work in DMs)

            def role_exists(award: RoleAward) -> Optional[Role]:
                """Check to ensure the role associated with the :class:`RoleAward` still exists in the guild. If it does, it returns that disnake role object for use"""
                return message.guild.get_role(award.role_id)  # type: ignore / `.guild` will always be :class:`disnake.Guild`
            ...
            if self.announce_level_up:
                ...
                lua: LevelUpAnnouncement = (
                    random.choice(self.level_up_announcement)
                ...
                if lua.level_up_channel_ids:
                    ...
                    for channel_id in lua.level_up_channel_ids:
                        channel = message.guild.get_channel(channel_id)  # type: ignore / `.guild` will always be :class:`disnake.Guild`
                        if channel:
                            channel_found = True
                            break
            ...
            if self._awards:
                try:
                    ...
                    guild_role_awards: List[RoleAward] = self._awards[message.guild.id]  # type: ignore / `.guild` will always be :class:`disnake.Guild`
                except KeyError:
                    return
                else:
    ...
    def _handle_amount_param(self, arg: Union[int, Sequence[int]]) -> None:
        """Simple check to ensure the proper types are being used for parameter "amount" in method :meth:`LevelingSystem.award_xp()`
        ...
        if isinstance(arg, (int, Sequence)):
            if isinstance(arg, int):
                ...
                if arg <= 0 or arg > 25:
                    raise LevelingSystemError('Parameter "amount" can only be a value from 1-25')
            else:
                # ensures there are only 2 values in the sequence
                if len(arg) != 2:
                    raise LevelingSystemError('Parameter "amount" sequence must only have two values')

                # ensures all values in the sequence are of type int
                for item in arg:
                    if not isinstance(item, int):
                        raise LevelingSystemError('Parameter "amount" sequence, all values must be of type int')

                # ensures all values in the sequence are >= 1 and <= 25
                for item in arg:
                    if item <= 0 or item > 25:
                        raise LevelingSystemError('Parameter "amount" sequence, all values can only be from 1-25')

                # ensures each value is unique
                if arg[0] == arg[1]:
                    raise LevelingSystemError('Parameter "amount" sequence expects both values to be unique')

                # ensures the first value is larger than the next
                if arg[1] < arg[0]:
                    raise LevelingSystemError('Parameter "amount" sequence expected value 1 to be larger than value 2')
        else:
            raise LevelingSystemError(f'Parameter "amount" expected int or Sequence, got {arg.__class__.__name__}')

    @db_file_exists
    @levels_exists
    @verify_levels_integrity
    async def award_xp(
        self, *, amount: Union[int, Sequence[int]] = [15, 25], message: Message, refresh_name: bool = True, **kwargs
    ) -> None:
        """|coro|
        ...
        Kwargs
        ------
        bonus: :class:`LevelingSystem.Bonus`
            Set the bonus values. Read the :class:`DisnakeLevelingSystem.Bonus` doc string for more details (defaults to :class:`None`)
        ...
        if any(
            ...
            # bonus XP
            bonus: Optional[LevelingSystem.Bonus] = kwargs.get("bonus")
            if bonus:
                for role_id in bonus.role_ids:
                    role: Optional[Role] = message.guild.get_role(role_id)  # type: ignore
                    if role in message.author.roles:  # type: ignore / This lib cannot operate with :class:`disnake.User` (DM's). It will always be :class:`disnake.Member`
                        if bonus.multiply:
            ...
            if not on_cooldown:
                ...
                async with self._connection.execute("SELECT * FROM leaderboard WHERE member_id = ? AND guild_id = ?", (member.id, member.guild.id)) as cursor:  # type: ignore
                    ...
                    if record:
                        ...
                        await cursor.execute(LevelingSystem._QUERY_NEW_MEMBER, (member.guild.id, member.id, str(member), 0, amount, amount))  # type: ignore
                        await self._connection.commit()  # type: ignore
