    ...
    async def wrapper(*args, **kwargs):
        ...
        if not any([instance._database_file_path, instance._connection]):
            raise DatabaseFileNotFound(
                'The database file was not found. Did you forget to connect to it first using "LevelingSystem.connect_to_database_file()"?'
            )
        ...
        # if it gets this far, that means :meth:`LevelingSystem.connect_to_file()` was ran, the connection
        # object was set, the file path was stored, and that file does end in ".db". This checks it again because
        # this check applies to various other methods that need to verify that the file exists. Using :meth:`LevelingSystem.connect_to_file()`
...
def levels_exists(func):
    """Ensures the "leaderboard" table exists in the "LevelingSystem.db" file"""
...
def verify_levels_integrity(func):
